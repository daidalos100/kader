...
test("tactics stay draggable, tied to lineups and support custom scenarios", async () => {
  ...
  for (const marker of [
    ...
    "tactics-ball", "onPointerDown", "onPointerCancel={cancelDraw}", "setPointerCapture(event.pointerId)", "onKeyDown", 'fetch("/api/lineup?lineupId=default"', "+ Neue Taktik", "onCreate", "onDuplicate", "onDelete",
...
test("calendar keeps its chronology from 07.07.2026 and stores local event overrides", async () => {
  ...
  for (const marker of ["calendarVisibleFrom", "next-calendar-event", "calendarOverrides", "Termin bearbeiten", "Google-Kalender bleibt unverändert", "TEILNAHME", "trainingEventIds"]) assert.ok(component.includes(marker), `missing ${marker}`);
