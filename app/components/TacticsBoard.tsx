...
function cloneLayout(layout: TacticLayout): TacticLayout { return { positions: Object.fromEntries(Object.entries(layout.positions).map(([id, point]) => [id, { ...point }])), ball: { ...layout.ball } }; }
function blankLayout(): TacticLayout { return { positions: Object.fromEntries(Object.entries(basePositions).map(([id, point]) => [id, { ...point }])), ball: { x: 50, y: 50 } }; }
...
function completeLayout(value: TacticLayout | undefined, scenario: TacticTemplate): TacticLayout {
  ...
  return { positions: Object.fromEntries(positions.map((position) => [position.id, value?.positions?.[position.id] ?? fallback.positions[position.id]])), ball: value?.ball ?? fallback.ball };
}
...
export default function TacticsBoard({ lineupId, eventTitle, tactics, onSave, onCreate, onRename, onDuplicate, onDelete }: {
  ...
  const [dragging, setDragging] = useState<DragTarget | null>(null); const [saving, setSaving] = useState(false); const [message, setMessage] = useState(""); const [presenting, setPresenting] = useState(false);
  ...
  useEffect(() => { const next = completeLayout(activeCustom?.layout ?? (activeEntry as TacticLayout | undefined), activeTemplate); setLayout(next); layoutRef.current = next; }, [scenario, tactics]);
  ...
  return <div className={`tactics-workspace${presenting ? " presenting" : ""}`}>
    ...
    </div><p className="tactics-help">Rote Positionen und Ball mit Maus oder Finger verschieben. Mit den Pfeiltasten ist eine exakte Positionierung möglich.</p>
