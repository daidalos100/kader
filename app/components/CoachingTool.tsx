...
function DiagnosticDetailsDialog({ profile, history, onClose, onEdit }: { profile: Profile; history: Diagnostic[]; onClose: () => void; onEdit: () => void }) {
  ...
  return <dialog ref={dialogRef} className="modal-backdrop" aria-labelledby="diagnostic-details-title" onCancel={(event) => { event.preventDefault(); onClose(); }} onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><form className="coach-modal diagnostic-details-dialog"><button className="close-dialog" type="button" aria-label="Dialog schließen" onClick={onClose}>×</button><p className="section-index">LEISTUNGSDIAGNOSTIK</p><h2 id="diagnostic-details-title">{profile.firstName}</h2>{latest?.metrics ? <div className="diagnostic-detail-list"><p>{latest.ageGroup} · Messung vom {latest.date}</p><DiagnosticMetricRow label="10 m Sprint" metric={latest.metrics.sprint10} unit="s" previous={null} /><DiagnosticMetricRow label="20 m Sprint" metric={latest.metrics.sprint20} unit="s" previous={null} /><DiagnosticMetricRow label="Laufgewandtheit" metric={latest.metrics.agility} unit="s" previous={null} /><DiagnosticMetricRow label="Dribbling" metric={latest.metrics.dribbling} unit="s" previous={null} /><div><span><strong>Shuttle Run</strong><small>Level {latest.metrics.shuttleRun.level ?? "—"}</small></span><b>{latest.metrics.shuttleRun.rating ?? "—"}</b></div><div><span><strong>Standweitsprung</strong><small>{latest.metrics.jump.attempts.map((attempt, index) => `V${index + 1}: ${attempt ?? "—"}`).join(" · ")}</small></span><b>{latest.metrics.jump.best ?? "—"} m<small>{latest.metrics.jump.rating ?? ""}</small></b></div></div> : <p className="history-empty">Noch keine Leistungsdiagnostik erfasst.</p>}<div className="modal-actions"><button className="text-button" type="button" onClick={onClose}>Schließen</button><button className="primary-button" type="button" onClick={onEdit}>Diagnostik erfassen</button></div></form></dialog>;
}
