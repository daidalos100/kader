...
type StatSortKey = "player" | "appearances" | "goals" | "assists" | "training" | "award";
...
const trainerQuotes = [
  ...
  { text: "Das Wichtigste für jeden Trainer ist die Verbindung zu seinen Spielern. Wir sind Menschen; Kommunikation und persönliche Gespräche sind im heutigen Fußball von entscheidender Bedeutung.", author: "Ole Gunnar Solskjær (Manchester United, 2024)" },
  { text: "Unabhängig von Taktik und Trainingsgestaltung müssen Trainer zuhören und gute Fragen stellen können. Spieler wollen an ihrer eigenen Entwicklung beteiligt sein.", author: "Lee Carsley (England U21, 2024)" },
  { text: "Für viele junge Spieler bist du vielleicht der erste Trainer ihres Lebens. Ihre Freude und ihre Bindung an das Spiel sind entscheidend; in einer freundlichen Umgebung entwickeln sie sich immer besser.", author: "Michael O’Neill (Nordirische Nationalmannschaft, 2024)" },
  { text: "Das Wichtigste für jeden Trainer, ob im Spitzenfußball oder bei Kindern, sind die Freude am Spiel und die Leidenschaft, anderen bei ihrer Entwicklung zu helfen und gemeinsam etwas aufzubauen.", author: "Jon Dahl Tomasson (Schwedische Nationalmannschaft, 2024)" },
  { text: "Manche Spieler brauchen klare Führung, andere Vertrauen und Freiheit. Deshalb muss man die Spieler unterschiedlich behandeln und verstehen, wie sie lernen und angesprochen werden müssen.", author: "Sabrina Wittmann (FC Ingolstadt 04, 2026)" },
  { text: "Ein Trainer muss Vertrauen zwischen sich und seinen Spielern aufbauen. Beständigkeit im Glauben an sie und im eigenen Verhalten ist für ihre Entwicklung von entscheidender Bedeutung.", author: "Arsène Wenger (FC Arsenal, 2024)" },
  { text: "Spieler möchten von ihrem Trainer als Menschen wahrgenommen werden und nicht nur als Nummer auf einem Trikot. Deshalb braucht es eine Beziehung, die den Teamgeist stärkt.", author: "Ottmar Hitzfeld (Schweizer Nationalmannschaft, 2013)" },
  { text: "Einzelne Persönlichkeiten zu einer Mannschaft zu formen, gelingt nicht ohne die menschliche Komponente.", author: "Ottmar Hitzfeld (Schweizer Nationalmannschaft, 2013)" },
  { text: "Das größte Geschenk für einen Trainer sind der Respekt, das Vertrauen und der Glaube seiner Spieler und seines Teams.", author: "Howard Wilkinson (Leeds United, 2019)" },
  { text: "Schaffe eine Atmosphäre des Vertrauens. Die Menschen, mit denen du arbeitest, müssen immer wissen, woran sie bei dir sind; sprich deshalb ehrlich mit ihnen.", author: "Dany Ryser (Schweizerischer Fußballverband, 2020)" },
  { text: "Im Fußball muss man den Menschen näherkommen, ein Umfeld des Vertrauens schaffen und anderen dabei helfen, selbst Entscheidungen zu treffen.", author: "Thomas Schaaf (Werder Bremen, 2013)" },
  { text: "Die Aufgabe eines Trainers besteht darin, Kinder über längere Zeit auf einer Reise des Lernens, Entdeckens und Erlebens von Freude zu begleiten. Das Ziel dieser Reise bestimmen jedoch sie selbst.", author: "Kris Van Der Haegen (Belgischer Fußballverband, 2019)" },
  { text: "Trainer müssen das Spiel an die Kinder anpassen und nicht die Kinder an das Spiel.", author: "Kris Van Der Haegen (Belgischer Fußballverband, 2019)" },
  { text: "Es geht darum, Interesse an dem zu zeigen, was ein Spieler als Mensch braucht, ihm zuzuhören und ihn zu unterstützen, besonders wenn er Schwierigkeiten hat.", author: "Sami Khedira (DFB, 2025)" },
  { text: "Der Spieler muss im Mittelpunkt stehen. Trainer müssen ihre Rolle verstehen und dürfen sich selbst nicht für wichtiger halten als den Spieler.", author: "Sami Khedira (DFB, 2025)" },
  { text: "Beim Coaching geht es darum, bessere Menschen zu entwickeln, nicht nur bessere Spieler.", author: "Pete Sturgess (The FA, 2012)" },
  { text: "Eine positive Umgebung ist das Fundament, auf dem Spieler lernen und Leistung zeigen können.", author: "The Football Association (England, 2020)" },
  { text: "Wir dürfen den Geist des Spiels niemals aus den Augen verlieren: Freude, Können, Kreativität, Widerstandsfähigkeit und fairer Wettbewerb.", author: "The Football Association (England, 2020)" },
  { text: "Als Fußballtrainer helfen wir Spielern dabei, sich in das Spiel zu verlieben und diese Liebe zu bewahren. Deshalb müssen wir ihnen zuhören; schließlich ist es ihr Spiel.", author: "Matthew Jones (The FA, 2020)" },
] as const;
...
export default function CoachingTool({ trainerName, trainerCanCorrectHistory }: { trainerName: string; trainerCanCorrectHistory: boolean }) {
  ...
  const [playerSeasonHistoryError, setPlayerSeasonHistoryError] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch("/api/player-season-history", { cache: "no-store" }).then(async (response) => {
        const data = await response.json() as { records?: PlayerSeasonHistory[]; error?: string };
        return {
          records: Array.isArray(data.records) ? data.records : [] as PlayerSeasonHistory[],
          error: response.ok ? null : data.error ?? "Historische Saisonwerte konnten nicht geladen werden.",
        };
      }),
    ])
      .then(([calendarEvents, coaching, lineupNames, savedEventLineups, historyPayload]) => {
        if (cancelled) return;
        setEvents(calendarEvents);
        setEventLineups(savedEventLineups);
        setPlayerSeasonHistoryError(historyPayload.error);
        setPlayerSeasonHistory(historyPayload.records.reduce<Record<string, PlayerSeasonHistory[]>>((grouped, record) => {
  ...
  const awardsByPlayer = (() => {
    ...
    Object.entries(state.matches).forEach(([eventId, match]) => {
      (match.goalEvents ?? []).forEach((goal) => {
        if (goal.deleted || !goal.assistId || goal.assistId === goal.scorerId) return;
        if (!rosterIds.has(goal.scorerId) || !rosterIds.has(goal.assistId)) return;
        const [playerA, playerB] = [goal.scorerId, goal.assistId].sort();
        const key = `${playerA}:${playerB}`;
        const pair = pairStats.get(key) ?? { playerA, playerB, count: 0, eventIds: new Set<string>() };
        pair.count += 1;
        pair.eventIds.add(eventId);
        pairStats.set(key, pair);
      });
    });

    const dreamDuos = [...pairStats.values()].filter((pair) => pair.count >= 2);
    const bestDreamDuoCount = Math.max(0, ...dreamDuos.map((pair) => pair.count));
    dreamDuos.filter((pair) => pair.count === bestDreamDuoCount).forEach((dreamDuo) => {
      const playerA = profiles.find((player) => player.id === dreamDuo.playerA);
  ...
  const overviewTraining = useMemo(() => {
    ...
    const calculate = (from: number, to: number) => {
      ...
      let present = 0;
      let recorded = 0;
      const playerRates = profiles.map((player) => {
        let playerPresent = 0;
        let playerRecorded = 0;
    ...
    const trend = current.rate === null || previous.rate === null ? "→" : current.rate > previous.rate ? "↑" : current.rate < previous.rate ? "↓" : "→";
    return { ...current, trend, previousRate: previous.rate };
  }, [calendarEvents, profiles, state.attendance]);
  const overviewBest = useMemo(() => {
    const namesForStat = (key: "goals" | "assists") => {
      const best = Math.max(0, ...statsRows.map((row) => row[key]));
      return best > 0 ? statsRows.filter((row) => row[key] === best).map((row) => row.player.firstName) : [];
    };
    const trainingBest = Math.max(0, ...statsRows.map((row) => row.participation ?? 0));
    const appearanceBest = Math.max(0, ...statsRows.map((row) => totalMatches ? Math.round((row.appearances / totalMatches) * 100) : 0));
    ...
    const entries = [
      { icon: "👑", title: "Training", names: trainingBest > 0 ? statsRows.filter((row) => row.participation === trainingBest).map((row) => row.player.firstName) : [] },
      ...
      { icon: "⛓️", title: "Kette", names: namesForAward("chain") },
      { icon: "🧱", title: "Bollwerk", names: namesForAward("bollwerk") },
      { icon: "⭐", title: "Scorer-Held", names: namesForAward("scorer-hero") },
    ].filter((entry) => entry.names.length > 0);
    return { entries };
  }, [awardsByPlayer, profiles, statsRows, totalMatches]);
  ...
  return (
    <main className="coach-shell">
            ...
            <section className="coach-view">
              ...
              <div className="overview-grid">
                ...
                <div className="overview-metrics">
                  <article className="metric-training"><p className="metric-kicker">TRAININGSQUOTE</p><strong>{overviewTraining.rate === null ? "—" : `${overviewTraining.rate}%`} <small className={`metric-trend metric-trend-${overviewTraining.trend === "↑" ? "up" : overviewTraining.trend === "↓" ? "down" : "stable"}`}>{overviewTraining.trend}</small></strong><div className="metric-footer"><span>Letzter Monat</span><span>Vormonat: {overviewTraining.previousRate === null ? "—" : `${overviewTraining.previousRate}%`}</span></div></article>
                  <article className="metric-best"><p className="metric-kicker">BESTWERTE</p><div className="metric-best-list">{overviewBest.entries.length ? overviewBest.entries.map((entry) => <span key={entry.title} title={`${entry.title}: ${entry.names.join(", ")}`}><b aria-hidden="true">{entry.icon}</b><strong>{entry.names.join(", ")}</strong></span>) : <span className="metric-best-empty">Noch keine Bestwerte erfasst.</span>}</div></article>
            ...
            <section className="coach-view">
              ...
              <div className="player-card-grid">
                {sortedCardRows.map(({ player, appearances, goals, assists, participation }) => {
                  ...
                  const bestStats = seasonStatBestByPlayer[player.id] ?? new Set<SeasonStatKey>();
                  const cardAwards: StatAward[] = [
                    ...(bestStats.has("training") ? [{ kind: "training-leader" as const, label: "👑", title: "Höchste positive Trainingsquote" }] : []),
                    ...(bestStats.has("appearances") ? [{ kind: "appearance-leader" as const, label: "🔥", title: "Höchste positive Einsatzquote" }] : []),
      ...
      {editingProfile && <ProfileDialog profile={editingProfile} seasonHistory={historyForProfile(editingProfile)} seasonHistoryError={playerSeasonHistoryError} onClose={() => setEditingProfile(null)} onSubmit={saveProfile} />}
...
function StaticPlayerCard({ profile, appearanceRate, goals, assists, participation, bestSeasonStatKeys: _bestSeasonStatKeys, history, awards, bestDisciplineKeys, onEdit, onDetails }: { profile: Profile; appearanceRate: number | null; goals: number; assists: number; participation: number | null; bestSeasonStatKeys: Set<SeasonStatKey>; history: Diagnostic[]; awards: StatAward[]; bestDisciplineKeys: Set<DiagnosticDisciplineKey>; onEdit: () => void; onDetails: () => void }) {
  return <article className="fc-card static-card"><div className="fc-card-inner"><section className="fc-face static-face"><div className="static-card-content"><div className="shirt-number">{profile.shirtNumber || "—"}</div><Image src={`/api/player-image?name=${encodeURIComponent(profile.firstName)}&v=20260723`} alt={profile.firstName} width={260} height={260} unoptimized /><div className="fc-name">{profile.firstName}</div><div className="fc-positions"><strong>{profile.primaryPosition || "POS"}</strong><span>{profile.secondaryPosition || "—"}</span></div><div className="static-card-awards" aria-label="Auszeichnungen">{awards.map((award) => <span key={`${award.kind}-${award.label}`} className={award.kind === "dream-duo" ? "static-card-award-duo" : "static-card-award-icon"} title={award.title}>{award.kind === "dream-duo" ? award.label : award.label.match(/^\p{Extended_Pictographic}/u)?.[0] ?? award.label}</span>)}</div><p className="section-index">SAISON 2026/27 · D1</p><div className="fc-metrics"><SeasonMetric label="Training" value={participation === null ? "—" : `${participation}%`} keyName="training" best={false} progress={participation} /><SeasonMetric label="Einsätze" value={appearanceRate === null ? "—" : `${appearanceRate}%`} keyName="appearances" best={false} progress={appearanceRate} /><SeasonMetric label="Tore" value={goals} keyName="goals" best={false} /><SeasonMetric label="Assists" value={assists} keyName="assists" best={false} /></div><DiagnosticOverview latest={history[0]} previous={history[1]} bestDisciplineKeys={bestDisciplineKeys} compact /></div><div className="static-card-actions"><button type="button" className="card-edit" onClick={onEdit}>Profil</button><button type="button" className="card-edit diagnostic-details-button" onClick={onDetails}>Leistung</button></div></section></div></article>;
}
...
function StatsTable({ rows, totalMatches, awardsByPlayer }: { rows: Array<{ player: Profile; appearances: number; goals: number; assists: number; participation: number | null; appearanceEvents: StatEventDetail[]; goalEvents: StatEventDetail[]; assistEvents: StatEventDetail[]; trainingEvents: StatEventDetail[] }>; totalMatches: number; awardsByPlayer: Record<string, StatAward[]> }) {
  ...
  const appearanceRateFor = (row: typeof rows[number]) => totalMatches ? Math.round((row.appearances / totalMatches) * 100) : null;
  const bestAppearanceRate = Math.max(0, ...rows.map((row) => appearanceRateFor(row) ?? 0));
  const bestTrainingRate = Math.max(0, ...rows.map((row) => row.participation ?? 0));
  const awardCountFor = (row: typeof rows[number]) => (awardsByPlayer[row.player.id] ?? []).length
    + Number(bestTrainingRate > 0 && row.participation === bestTrainingRate)
    + Number(bestAppearanceRate > 0 && appearanceRateFor(row) === bestAppearanceRate)
    + Number(bestGoals > 0 && row.goals === bestGoals)
    + Number(bestAssists > 0 && row.assists === bestAssists);
  ...
  const sortedRows = [...rows].sort((left, right) => {
    ...
    const leftValue = sortKey === "player" ? left.player.firstName : sortKey === "training" ? left.participation ?? -1 : sortKey === "award" ? awardCountFor(left) : left[sortKey];
    const rightValue = sortKey === "player" ? right.player.firstName : sortKey === "training" ? right.participation ?? -1 : sortKey === "award" ? awardCountFor(right) : right[sortKey];
  ...
  return <><table className="stats-table" aria-describedby="stats-scroll-hint"><thead><tr>{header("player", "Spieler:in")}{header("training", "Training")}{header("appearances", "Einsätze")}{header("goals", "Tore")}{header("assists", "Assists")}{header("award", "Award")}</tr></thead><tbody>{sortedRows.map((row) => {
    ...
    const leaderAwards: StatAward[] = [
      ...(bestTrainingRate > 0 && row.participation === bestTrainingRate ? [{ kind: "training-leader" as StatAwardKind, label: "👑", title: "Höchste positive Trainingsquote" }] : []),
      ...(bestAppearanceRate > 0 && appearanceRate === bestAppearanceRate ? [{ kind: "appearance-leader" as StatAwardKind, label: "🔥", title: "Höchste positive Einsatzquote" }] : []),
...
function ProfileDialog({ profile, seasonHistory, seasonHistoryError, onClose, onSubmit }: { profile: Profile; seasonHistory: PlayerSeasonHistory[]; seasonHistoryError: string | null; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  const dialogRef = useRef<HTMLDialogElement>(null);
  useEffect(() => { const dialog = dialogRef.current; dialog?.showModal(); return () => dialog?.close(); }, []);
  return <dialog ref={dialogRef} className="modal-backdrop" aria-labelledby="profile-dialog-title" onCancel={(e) => { e.preventDefault(); onClose(); }} onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}><form className="coach-modal" onSubmit={onSubmit}><button className="close-dialog" type="button" aria-label="Dialog schließen" onClick={onClose}>×</button><p className="section-index">SPIELERPROFIL</p><h2 id="profile-dialog-title">{profile.firstName}</h2><div className="form-grid"><label>Trikotnummer<input autoFocus name="shirtNumber" type="number" min="0" max="999" defaultValue={profile.shirtNumber} /></label><label>Hauptposition<select name="primaryPosition" defaultValue={profile.primaryPosition}><option value="">Bitte wählen</option>{positionOptions.map((position) => <option key={position}>{position}</option>)}</select></label><label>Ersatzposition<select name="secondaryPosition" defaultValue={profile.secondaryPosition}><option value="">Keine</option>{positionOptions.map((position) => <option key={position}>{position}</option>)}</select></label><label>Starker Fuß<select name="strongFoot" defaultValue={profile.strongFoot}><option value="">Bitte wählen</option><option value="left">Links</option><option value="right">Rechts</option><option value="both">Beide</option></select></label><label className="wide">Spielerpersönlichkeit<textarea name="personality" defaultValue={profile.personality} rows={4} placeholder="Stärken, Rolle im Team, Coaching-Hinweise …" /></label></div><section className="profile-history-panel"><p className="section-index">VERGANGENE SAISON</p>{seasonHistory.length ? <SeasonHistoryOverview records={seasonHistory} /> : <p className="profile-history-empty">{seasonHistoryError ?? "Für die Saison 2025/26 wurden noch keine Werte importiert."}</p>}</section><button className="primary-button" type="submit">Profil speichern</button></form></dialog>;
}
