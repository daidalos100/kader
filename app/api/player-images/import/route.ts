...
export async function POST(request: Request) {
  ...
  if (!url || !key) {
    return Response.json({ error: "Supabase ist nicht konfiguriert." }, { status: 503 });
  }

  const form = await request.formData();
  const files = form.getAll("files").filter((item): item is File => item instanceof File);
  if (!files.length || files.length > 30) {
    return Response.json({ error: "Es werden 1 bis 30 Bilder erwartet." }, { status: 400 });
  }
  ...
  if (!bucketResponse.ok && bucketResponse.status !== 409) {
    return Response.json({ error: "Der private Bilder-Bucket konnte nicht angelegt werden." }, { status: 502 });
  }
  ...
  for (const file of files) {
    ...
    const response = await fetch(
      `${url}/storage/v1/object/player-images/${encodeURIComponent(file.name)}`,
      {
        method: "POST",
    ...
    if (!response.ok) {
      return Response.json({ error: `Upload fehlgeschlagen: ${file.name}` }, { status: 502 });
    }
